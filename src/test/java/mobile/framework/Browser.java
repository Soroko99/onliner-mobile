...
public class Browser {
    ...
    private static MobileDriver<WebElement> browser = DeviceFactory.getDriver();
    ...
    public static Browser getInstance() {
        if (instance == null) {
            initProperties();
            try {
                browser.manage().timeouts().implicitlyWait(IMPLICITLY_WAIT, TimeUnit.SECONDS);
            } catch (Exception e) {
    ...
    public void waitForPageToLoad() {
        WebDriverWait wait = new WebDriverWait(browser, Long.parseLong(getTimeoutForPageLoad()));
        try {
